...
import { getCostRecordsList } from '@/api/detailList';
...
export default () => {
  ...
  const handleRequest = async (params: ParamsType) => {
    ...
    try {
      const result = await getCostRecordsList(params1);
  ...
  const handleExport = async () => {
    ...
    try {
      // TODO:导出
      // const result = downloadByPost('/api/platform/api/human/organizational/depart/export', queryParams, '费用明细')
      setIsSpinning(false);
    }
    catch (e) {
      setIsSpinning(false);
