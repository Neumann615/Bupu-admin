...
import { getPayRecordsList } from '@/api/detailList';
...
export default () => {
  ...
  const handleRequest = async (params: ParamsType) => {
    ...
    try {
      const result = await getPayRecordsList(params1);
  ...
  const handleExport = async () => {
    ...
    try {
      // TODO:导出
      // const result = downloadByPost('/api/platform/api/human/organizational/depart/export', queryParams, '消费明细')
      setIsSpinning(false);
    }
    catch (e) {
      setIsSpinning(false);
